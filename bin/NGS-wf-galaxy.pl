...
my $opt_file     = $opts{t};
my $opt_command_in = $opts{T}; #### ':' delimited entries, e.g. opt1:opt2:opt3
...
cd $t_sample_id
mkdir $t_job_id
#if ! [ -f $f_start ]; then date +\%Y\%m\%d\%k\%M\%S.\%N > $f_start;  fi
if ! [ -f $f_start ]; then date +\%s > $f_start;  fi
$t_command
$v_command
#date +\%Y\%m\%d\%k\%M\%S.\%N > $f_complete
date +\%s > $f_complete
times >> $f_cpu
EOD
      close(TSH);
...
sub usage {
<<EOD;
NGS workflow tools to run multiple samples under Linux systems

To run workflow: 
    $0 -s sample_file -i workflow_file
To execute a single task, without running the full workflow
    $0 -s sample_file -i workflow_file -J task


Options:
    -i workflow file
    -s file of sample data
    -J optional tasks
    ...
    -Q queue system, default PBS
        can be PBS, SGE
    -R run sub sets of workflows, sub sets are defined in workflow file
       e.g. -R set1 or -R set1,set2
    -j run sub sets of jobs, if job dependancy is not solved, job won't run
